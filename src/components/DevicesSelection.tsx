...
const DevicesSelection = ({ devices }: DevicesSelectionProps) => {
  ...
  function onSave() {
    ...
    Axios.get(address + '/select-devices', {
      headers: {
        'devices': state.map(device => device.name).join(',')
      }
    }).then(_ => {
  ...
  return (
    <div id='devices-selection' className='col2'>
      ...
      <div id='selection-buttons2' className='col2'>
        <button id='selection-save' onClick={onSave}>Save</button>
        <button id='selection-cancel' onClick={onCancel}>Cancel</button>
      </div>
    </div>
  )
}
